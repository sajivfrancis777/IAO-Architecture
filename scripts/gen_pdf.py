...
<script type="module">
  ...
  // Lazy-render: only render Mermaid diagrams when they scroll into view
  ...
  // Force-render ALL diagrams (used before print and by PDF button)
  ...
  // Observe all .mermaid divs once DOM is ready
  ...
  // Before print (Ctrl+P or window.print): render all diagrams first
  window.addEventListener('beforeprint', function() {
    // Synchronous fallback — render whatever we can
    var unrendered = [];
    document.querySelectorAll('.mermaid').forEach(function(el) {
      if (!rendered.has(el)) {
        unrendered.push(el);
  ...
  // Expose for the PDF button to call
  window._renderAllMermaid = renderAllMermaid;

  // PDF download: render all diagrams, then print
