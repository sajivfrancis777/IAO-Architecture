...
@page {
    size: A4;
    margin: 0;   /* zero page-margin eliminates browser-default header (date/title) and footer (URL/page-number) */
}
body {
...
/* ---- Global print-only footer (fixed to bottom of every printed page) ---- */
.print-footer {
...
@media print {
    body { padding: 12mm 15mm 22mm 15mm; max-width: none; }
    ...
    /* Hide per-section footers in print — they don't reliably sit at
       the bottom of multi-page sections.  A global fixed footer below
       appears on every physical page instead. */
    .page-footer {
        display: none !important;
    }

    /* Global fixed footer — Chrome/Edge render position:fixed elements
       at the same position on every printed page.
    ...
    /* Page sections: keep flex for short pages, but remove min-height
       so multi-page sections flow naturally. */
    .page-section {
        display: block;
        box-sizing: border-box;
    }
    ...
    h2 { margin-top: 14px; margin-bottom: 6px; font-size: 16pt; }
    h3 { margin-top: 10px; margin-bottom: 4px; font-size: 13pt; }
    h4 { margin-top: 8px; margin-bottom: 3px; font-size: 11pt; }
    table { margin: 6px 0; font-size: 8pt; table-layout: fixed; word-wrap: break-word; }
    th { padding: 3px 4px; }
    td { padding: 3px 4px; }
    blockquote { margin: 6px 0; padding: 4px 12px; font-size: 9pt; }
    p { margin: 4px 0; }
    ul, ol { margin: 4px 0; }
    .mermaid { margin: 6px 0; }
}
"""
...
/* Subgraph padding — space between header and content */
...
<!-- Mermaid 11.14.0: Eager rendering on page load.
     All diagrams render once during load so scrolling stays smooth
     and Print / PDF captures every diagram correctly. -->
<script type="module">
  ...
  // Wait for ALL diagrams to finish rendering, then trigger print.
  // startOnLoad renders eagerly but asynchronously — some large diagrams
  // may still be parsing when the user clicks "Download PDF".
  window._printPDF = async function() {
    // Re-run mermaid on any divs that haven't rendered yet (no SVG child)
    var unrendered = [];
    document.querySelectorAll('.mermaid').forEach(function(el) {
      if (!el.querySelector('svg')) unrendered.push(el);
    });
    if (unrendered.length > 0) {
      await mermaid.run({ nodes: unrendered });
    }
    // Small delay to let the browser paint the final SVGs
...
<body>
<button class="pdf-download-btn" onclick="window._printPDF()" title="Print / Save as PDF">&#128196; Download PDF</button>
{html_body}
<div class="footer">
    IAO Architecture Document | Intel IDM 2.0 | Generated by IAO Architecture Pipeline
</div>
<div class="print-footer">
    <span><a href="#toc">&uarr; Back to TOC</a></span>
    <span><strong>{title}</strong></span>
    <span>IAO Architecture Pipeline &mdash; Intel Confidential</span>
</div>
</body>
</html>"""
    return html
...
def convert_file(md_path: Path, html_only: bool = False) -> tuple[Path | None, Path | None]:
    ...
    # Build a readable title from the filename (fallback)
    title = md_path.stem
    for suffix in ("-Architecture", "-RICEFW-Tracker", "-Testing-Report"):
        title = title.replace(suffix, "")
    title = title.strip()

    # Try to extract the full capability name from the title-page <h1>
    h1_match = re.search(r'<h1[^>]*>([^<]+)</h1>', md_content)
    if h1_match:
