<div style="text-align:center; padding-top:20px;">
  ...
  Capability L-040 · L Logistics and Inventory Management - FTS (IF)</p>
...
flowchart TD
    ...
    subgraph lane_0["Warehouse Operator"]
        n1["Receive Manifest Against Stock Transfer Order (STO) or Production Order"]
        n2(["fa:fa-stop Manifest Received"])
        n3["Obtain and Record Shipping Notification"]
    end
    n3 --> n1
    n1 --> n2
    class n2 endEvt
...
flowchart TD
    ...
    classDef gateway fill:#FFF9C4,stroke:#F57F17,stroke-width:2px,color:#E65100
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph lane_0["LOG IF - Warehouse Operator"]
        n1["fa:fa-user Post Goods Movement Against Reservation Order for 262 Movement Type"]
        n2["fa:fa-user Pick Goods from Passthrough Location"]
        n3["fa:fa-user Reject Inbound Delivery with Reason code"]
        n4["fa:fa-user Perform Goods Receipt"]
        n5[["fa:fa-cog Replicate Inbound Delivery in EWM"]]
        n6["Create Inbound Delivery"]
        n7["Perform Physical Inspection, Count and Verify Goods"]
        n8(["fa:fa-stop Putaway Completed"])
        n9["Return Unused Parts to Stock"]
        n10{{"fa:fa-code-branch Validation of Goods per reason codes?"}}
        n11{{"fa:fa-code-branch exclusiveGateway"}}
        n12[["fa:fa-folder-open Determine Put-away Location - FTS (IF)_R3"]]
        n13[["fa:fa-folder-open Confirm/Adjust Location - FTS (IF)_R3"]]
    end
    n9 --> n1
    n1 --> n6
    n6 --> n11
...
flowchart TD
    ...
    subgraph lane_0["LOG IF - Warehouse Operator"]
        ...
        n2["fa:fa-user Unload Physical Boxes via RF Scanner or SAP GUI"]
        n3["fa:fa-user Perform Good Receipt via Scanning Labels of Physical Boxes in RF Scanner..."]
        n4["fa:fa-user Create Outbound Delivery"]
        n5["fa:fa-user Perform Picking from Staging Area (GI-ZONE)"]
        n6["fa:fa-user Confirm Warehouse Task"]
        n7[["fa:fa-cog Distribute Inbound Delivery to EWM"]]
        n8[["fa:fa-cog Distribute Outbound Delivery with Delivery Priority Indicator"]]
        n9[["fa:fa-cog Post Goods Issue"]]
        n10["Count and Verify Physical Boxes"]
        n11["Transfer Cross Dock Product from GR-ZONE to GI-ZONE"]
        n12["Load Goods on Truck"]
        n13(["fa:fa-play Cross dock Creation Initiated"])
        n14(["fa:fa-play Initiate Outbound Delivery Creation"])
        n15(["fa:fa-stop End of Cross Docking Process"])
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph lane_0["Warehouse Operator"]
        n1["Transport Physical Boxes to Putaway Storage Bins"]
        n2(["fa:fa-play Stock Transport Initiated"])
        n3(["fa:fa-stop Stock Transported to Putaway Area"])
    end
    n2 --> n1
    n1 --> n3
    class n2 startEvt
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph lane_0["Warehouse Operator"]
        n1["fa:fa-user Verify Material Location and Update of Inventory"]
        n2["fa:fa-user Record Storage Bin Location of Material via Label"]
        n3[["fa:fa-cog Putaway Task Confirmed"]]
        n4(["fa:fa-stop Putaway Completed"])
        n5["Confirm/ Adjust Location"]
    end
    n1 --> n4
    n5 --> n3
    n3 --> n2
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph lane_0["Warehouse Operator"]
        n1[["fa:fa-cog A-010 Update the Inventory into the System"]]
        n2[["fa:fa-cog A-020 Display Stock Overview"]]
        n3(["fa:fa-stop Inventory Updated"])
        n4["Confirm/Adjust Location (IF)"]
    end
    n2 --> n3
    n1 --> n2
    n4 --> n1
