<div style="text-align:center; padding-top:20px;">
  ...
  Capability PE-070 · PE Manage Plant, Equipment and Facilities (IF)</p>
...
flowchart TD
    ...
    subgraph Boundary Apps
        n1[["fa:fa-cog Detect Tool Down by Boundary App"]]
        n2[["fa:fa-cog Auto Generate Notification"]]
        n3(["fa:fa-play Request Received for Maintenance Work"])
        n4["Create and Maintain Notification"]
    end
    n3 --> n1
    n1 --> n2
    n2 --> n4
...
flowchart TD
    ...
    subgraph FTS IF - Maintenance Planner
        n2[["fa:fa-cog Create Notification in SAP S/4 HANA"]]
        n3["Send Email from SAP S/4 HANA"]
    end
    subgraph Factory Supervisor
        n1[["fa:fa-cog Auto Generate Notification"]]
        n4["Review Notification and Validate Request for Maintenance"]
        n5["Identify Maintenance Work Required"]
        n6{{"fa:fa-code-branch Source?"}}
...
flowchart TD
    ...
    subgraph FTS IF - Maintenance Technician
        n1["fa:fa-user Review Email Notification"]
        n2["fa:fa-user Review and Approve Notification"]
        n3["Create and Maintain Maintenance Order"]
        n4["Perform Field Inspection to Create Request"]
        n5["Create and Maintain Notification"]
        n6{{"fa:fa-code-branch System Generating Notification?"}}
    end
    n5 --> n6
    n1 --> n3
    n2 --> n4
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph FTS IF - Maintenance Planner
        n1["fa:fa-user Review Planned Maintenance Orders Created for Equipment"]
        n2["Create and Maintain Maintenance Order"]
        n3["Process Notification"]
    end
    n3 --> n1
    n1 --> n2
    class n1 userTask
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph Factory Supervisor
        n1["fa:fa-user Perform MES/AutoTask Diagnostics"]
        n2["fa:fa-user Review Checklists"]
        n3["Process Notification"]
        n4["Review Notification and Validate Request for Maintenance"]
    end
    n4 --> n1
...
flowchart LR
    ...
    subgraph FTS IF - Batch User
        n4[["fa:fa-cog Update Claim Document Status"]]
        n5[["fa:fa-cog Create Claim Document (Draft)"]]
        n6[["fa:fa-cog Create Return PR / PO"]]
        n7[["fa:fa-cog Transmit Claim Document"]]
        n8[["fa:fa-cog Update Warranty Information in Equipment and Master Warranty Table for..."]]
        n14(["fa:fa-stop Claim Document Closed"])
        n15(["fa:fa-stop Corrective/Breakdown Process Continued"])
        n16(["fa:fa-stop No Claim Processed"])
        n17(["fa:fa-stop Warranty Updated"])
        n22["Warranty Check for Equipment Initiated"]
        n23["intermediateThrowEvent"]
        n28{{"fa:fa-code-branch Finalized Claimed Status ?"}}
        n29{{"fa:fa-code-branch exclusiveGateway"}}
        n30{{"fa:fa-code-branch exclusiveGateway"}}
        n31{{"fa:fa-code-branch In QUAL?"}}
        n32{{"fa:fa-code-branch Item/Part Under Warranty ?"}}
        n33{{"fa:fa-code-branch exclusiveGateway"}}
        n34{{"fa:fa-code-branch In Sustained / Extended ?"}}
        n35{{"fa:fa-code-branch exclusiveGateway"}}
        n36{{"fa:fa-code-branch Is Part Covered ?"}}
        n37{{"fa:fa-code-branch Equipment Under Warranty ?"}}
        n41{{"fa:fa-arrows-alt parallelGateway"}}
    end
    subgraph FTS IF - PM Maintenance Technician
        n1["fa:fa-user Create Corrective / Break Down Order"]
        n2["fa:fa-user Perform Warranty Claim Validation Pop-up"]
        n11["Maintain Flag for Warranty Indicator"]
        n12(["fa:fa-play Warranty Check Initiated"])
        n13(["fa:fa-stop Corrective/Breakdown Process Continued"])
        n20["System up and Status Running"]
        n21["TECO (Technically Completed)"]
        n25{{"fa:fa-code-branch Is tool ready to be brought up to production ?"}}
        n26{{"fa:fa-code-branch Do parts have MMID?"}}
        n27{{"fa:fa-code-branch Is equipment under warranty?"}}
        n39{{"fa:fa-arrows-alt Warranty pop-up​ ? (with warranty checkbox indicator determination after..."}}
        n40{{"fa:fa-arrows-alt parallelGateway"}}
        n42{{"fa:fa-arrows-alt inclusiveGateway"}}
    end
    subgraph Finance Manager
        n3[["fa:fa-cog Post Financial Credit (W/refer to claim)"]]
        n19["Send Credit Memo to Supplier"]
    end
    subgraph PTP IF Batch user
        n9[["fa:fa-cog Create Free Goods PR"]]
        n10[["fa:fa-cog System generates PR/PO"]]
        n18(["fa:fa-stop Free Goods PR/PO Process End"])
        n24["Create and Maintain Maintenance Order"]
    ...
    n9 --> n18
    n4 --> n14
    n28 -->|"Approved with Free Goods PR"| n9
    n28 -->|"Approved with 
Financial Credit"| n3
    n25 -->|"Yes"| n20
    n29 --> n7
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph Factory Supervisor
        n1["fa:fa-user Check if Diagnostics Fixed Issue"]
        n2["Review Equipment Maintenance Planned Orders"]
        n3["Close Notification"]
        n4["Perform Field Inspection to Create Request"]
        n5{{"fa:fa-code-branch Diagnostics Fixed Issue?"}}
    end
    n4 --> n1
    n5 -->|"Yes"| n3
    n5 -->|"No"| n2
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph Factory Supervisor
        n1["fa:fa-user Set Status to Resolved"]
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph Boundary Apps
        n2["fa:fa-user Create Work Order"]
        n7["Process Notification"]
    end
    subgraph FTS IF - PM Batch User
        n1["fa:fa-user Run Chron Job to Find Open WO"]
        n3[["fa:fa-cog Convert Notification into Work Order"]]
        n4[["fa:fa-cog Replicate Work Order in S4 from XEUS"]]
        n5["Identify Required Spare Parts"]
        n6["Review Notification and Validate Request for Maintenance"]
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph FTS IF - Maintenance Planner
        n1["fa:fa-user Create Maintenance Strategy"]
        n2["fa:fa-user Assign Maintenance Strategy to Task List"]
        n3["fa:fa-user Maintain Maintenance Package"]
        n4["fa:fa-user Display Strategies Through Task List"]
        n5["Create Maintenance Plan"]
        n6["Create and Maintain Maintenance Order"]
    end
    n6 --> n1
    n1 --> n2
    n3 --> n4
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph FTS IF - Maintenance Planner
        n1["fa:fa-user Create Single Cycle Maintenance Plan-Counter/Non-Counter"]
        n2["fa:fa-user Change/Mass Change Maintenance Plan"]
        n3["fa:fa-user Set Deletion Flag"]
        n4["fa:fa-user Determine Maintenance Plan Type"]
        n5["fa:fa-user Create Strategy-Based Maintenance Plan (Performance/Time Based)"]
        n6["fa:fa-user Create Multiple Counter/Dimension-Based Maintenance Plan"]
        n7["fa:fa-user Schedule Maintenance Plan"]
        n8(["fa:fa-stop Maintenance Plan Deleted"])
        n9["Create and Maintain Maintenance Order"]
        n10["Create and Maintain Notification"]
        n11["Review Maintenance Plan"]
        n12{{"fa:fa-code-branch Single Cycle/ Strategy/ Multiple Counter?"}}
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph Boundary Apps
        n1["fa:fa-user Identify Maintenance Tasks Required for Job"]
        n2["fa:fa-user Identify Internal and External Resources (FSE)"]
        n3["fa:fa-user Identify Maintenance Skill Required to Perform Job"]
        n4["fa:fa-user Identify Parts Required for Job from Spec"]
        n5["fa:fa-user Add/Update Task List Job Plan in Work Order"]
        n6["fa:fa-user Link Steps from Task List If Required"]
        n7["Identify Required Spare Parts"]
        n8["Consult Equipment Files/Data"]
        n9["Maintain General Task List"]
...
flowchart TD
    ...
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph FTS IF - PM Maintenance Technician
        n1["fa:fa-user Manually Add Stock Items to Work Order"]
        n2["fa:fa-user Reserve Stock Automatically Items Available on Site"]
        n3["Identify Asset Acquisition"]
        n4["Define Requirements for Each Operation and Create Job Plan"]
    end
    n4 --> n1
    n1 --> n2
    n2 --> n3
