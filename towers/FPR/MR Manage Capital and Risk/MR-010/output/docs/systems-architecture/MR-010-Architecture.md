...
flowchart TD
    ...
    subgraph lane_0["MDG Analyst"]
        n1["fa:fa-user Check the Error in WE02/WE05 (IDOC)"]
        n2["fa:fa-user Execute WE19 to Test and Execute the IDOC"]
        n3["fa:fa-user Check the Exchange Rates in OB08 Transaction"]
        n4["fa:fa-user Upload Correct Exchange Rates using App Import Foreign Exchange Rates"]
        n5["fa:fa-user Verify the Results"]
        n6["fa:fa-user Analyze and Reprocess Rates from MDG"]
        n10[["fa:fa-cog Replicate Rates to CFIN, IP, IF through IDOC"]]
        n11[["fa:fa-cog Replicate Rates to ECA through SLT"]]
        n15(["fa:fa-stop Rates Loaded Successfully"])
        n17{{"fa:fa-code-branch Can the Error be Corrected in IDOC/MDG?"}}
        n18{{"fa:fa-code-branch Are the Exchange Rates Correct?"}}
        n19{{"fa:fa-code-branch exclusiveGateway"}}
        n20{{"fa:fa-code-branch exclusiveGateway"}}
        n21{{"fa:fa-code-branch Rates loaded successfully?"}}
        n22{{"fa:fa-code-branch Can Rates be Reprocessed within SLA?"}}
    ...
    subgraph lane_1["Treasury IT Analyst"]
        n7["fa:fa-user Check the System for which the Error Message is Raised"]
        n8["fa:fa-user Work with Mulesoft Team to Resolve the Issue"]
        n9["fa:fa-user Work with MuleSoft Team to Reprocess/Process IDOC"]
        n12[["fa:fa-cog Re-execute Bloomberg Jobs"]]
        n13[["fa:fa-cog Email Alert Received from Mulesoft to MDG and Treasury IT"]]
        n14(["fa:fa-play Alert needs to be Received from Mulesoft"])
        n16["Email Alert Received"]
        n25{{"fa:fa-code-branch Re-execution of Bloomberg Jobs Required?"}}
        n26{{"fa:fa-code-branch Was the IDOC Received in MDG?"}}
...
flowchart TD
    ...
    classDef gateway fill:#FFF9C4,stroke:#F57F17,stroke-width:2px,color:#E65100
    classDef subProc fill:#F3E5F5,stroke:#7B1FA2,stroke-width:2px,color:#4A148C
    subgraph lane_0["Close and Reporting IT Analyst"]
        n1["fa:fa-user Review Bank Statement Load in Monitor"]
        n2["fa:fa-user Investigate the EBS Job Failure"]
        n3["fa:fa-user Retrigger EBS Batch Jobs"]
        n4["fa:fa-user Upload Bank Statement Manually with Firefighter Access"]
        n5["fa:fa-user Upload Lockbox File Manually with Firefighter Access"]
        n10[["fa:fa-cog Load BAI2 Lockbox via CFIN Daily Batch Jobs"]]
        n11[["fa:fa-cog Load CAMT053 EBS via CFIN Daily Batch Jobs"]]
        n15["Connect with Banking Partner to Retrieve Missing Files"]
        n17(["fa:fa-stop Batch Jobs Retriggered"])
        n18(["fa:fa-stop Files Archived in SAP Directory by MBC"])
        n21["Reconcile Account Statements"]
        n22["Email Notification Triggered"]
        n26{{"fa:fa-code-branch Missing Files?"}}
        n27{{"fa:fa-code-branch CFIN Batch Job Failure?"}}
        n28{{"fa:fa-code-branch exclusiveGateway"}}
        n29{{"fa:fa-code-branch Manual Upload Required?"}}
        n30{{"fa:fa-code-branch EBS or Lockbox?"}}
        n31{{"fa:fa-code-branch exclusiveGateway"}}
        n32{{"fa:fa-code-branch exclusiveGateway"}}
        n39{{"fa:fa-arrows-alt parallelGateway"}}
    end
    subgraph lane_1["Treasury IT Analyst"]
        n6["fa:fa-user View MBC Monitor for Load Errors"]
        n7["fa:fa-user Take Corrective Action"]
        n8["fa:fa-user Take Corrective Actions"]
        n9["fa:fa-user Retrigger the Job"]
        n12[["fa:fa-cog Process Files in CFIN"]]
        n13[["fa:fa-cog Receive Files in FD Share Folder in CFIN"]]
        n14[["fa:fa-cog Pick Files from FD Share Folder (Picked by Hubspan)"]]
        n16(["fa:fa-play Bank Statement Received by MBC (CAMT053, CAMT052, BAI2 Lockbox)"])
        n19(["fa:fa-stop Error Resolved"])
        n20(["fa:fa-stop Files Picked and Sent to Quantum"])
        n23["Email Notification Triggered"]
        n24["Email Notification Triggered"]
        n25["CAMT052 and CAMT053 File Sent to Quantum via Hubspan"]
        n33{{"fa:fa-code-branch exclusiveGateway"}}
        n34{{"fa:fa-code-branch Files Received in MBC?"}}
        n35{{"fa:fa-code-branch Errors Found?"}}
        n36{{"fa:fa-code-branch exclusiveGateway"}}
        n37{{"fa:fa-code-branch exclusiveGateway"}}
        n38{{"fa:fa-code-branch Is the Bank Statement Going to FD Share or CFIN?"}}
    ...
    n30 -->|"Lockbox"| n5
    n25 --> n13
    n38 -->|"CAMT052 and CAMT053"| n25
    n9 --> n36
    n38 -->|"CAMT053, BAI2(Lockbox)"| n40
    n41 --> n35
    n13 --> n14
    n34 -->|"Yes"| n38
    n14 --> n20
...
flowchart LR
    ...
    subgraph lane_0["AR Cash Applier"]
        n2["fa:fa-user Perform Manual Match and Clear"]
        n7(["fa:fa-stop Statements Reconciled"])
    end
    subgraph lane_1["Close And Reporting Accounting Analyst"]
        n1["fa:fa-user Perform Manual Match and Clear"]
        n6(["fa:fa-stop Reconcile Account Statements Completed"])
        n8["Automatic Batch Job for BlackLine Connector Trigerred"]
        n9["Receive Account Statements"]
        n10{{"fa:fa-code-branch Are the Items Matched and Cleared Automatically (Using search strings, DMEE..."}}
        n11{{"fa:fa-code-branch Automatically Reconciled?"}}
        n12{{"fa:fa-code-branch exclusiveGateway"}}
        n13{{"fa:fa-code-branch Is it Lockbox or EBS Statement?"}}
    end
    subgraph lane_2["Close and Reporting IT Analyst"]
        n3["fa:fa-user Export Output from 'Display GL Items'"]
        n4["fa:fa-user Export Output from 'Reprocess Bank Statement Items'"]
        n5["fa:fa-user Load the Output"]
